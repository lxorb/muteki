...
class BuilderAgent(Agent):
    def __init__(self, strategy_methods):
        super().__init__()
        self.strategy = list(strategy_methods or [])
        # -> ordered `s_...` callables to try for this builder, highest priority first
    ...
    def u_execute_strategy(self) -> bool:
        ...
        `strategy_methods` is treated as a priority-ordered list of zero-argument
        callables, typically bound `s_...` methods on this agent. On a fresh
        turn the executor starts at index `0`. If the previous turn ended
        before this method returned, execution resumes at the first method that
        has not completed yet, which is the entry after
        `last_executed_index`. The index is only advanced after a method
        returns, so an interrupted step is retried on the next turn. Execution
        stops at the first truthy result and returns whether any strategy
        method acted.
        """
        ...
        if self.last_turn_completed:
            ...
            start_index = self.last_executed_index + 1
            if start_index < 0 or start_index >= len(self.strategy):
                start_index = 0

        self.last_turn_completed = False
        self.bb_last_turn_completed = False
        for index in range(start_index, len(self.strategy)):
            acted = bool(self.strategy[index]())
            self.last_executed_index = index
            self.last_strategy_index = index
            if acted:
                self.last_turn_completed = True
                self.bb_last_turn_completed = True
                return True
